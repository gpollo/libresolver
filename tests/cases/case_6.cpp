...
namespace tests::cases::case_6 {

TEST_CASE("pattern tree should match pattern case 6", "[libresolver::cases::case_6]") {
    x86::engine engine;
    dummy::memory memory;
    ...
    SECTION("example 1") {
        const unsigned char bytes[] =
            ...
            "\x83\x78\x32\x06"             /* cmpl   $6,50(%rax) */
            "\x0f\x87\xe0\xee\xff\xff"     /* ja     10 */
            "\x8b\x40\x32"                 /* mov    50(%rax),%eax */
        ...
        REQUIRE(context.get(value::VALUE_3).value_or(-1) == 50);
        REQUIRE(context.get(value::VALUE_4).value_or(-1) == 18446744073709547249);
        REQUIRE(context.get(value::VALUE_5).value_or(-1) == 6);
        REQUIRE(context.get(value::VALUE_6).value_or(-1) == 50);
