...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, ...)                                                               \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                                                         \
        Catch::makeTestInvoker(Function), CATCH_INTERNAL_LINEINFO, "", Catch::NameAndTags{__VA_ARGS__}); /* NOLINT */ \
    CATCH_INTERNAL_UNSUPPRESS_GLOBALS_WARNINGS
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                                   \
    do {                                                                                                         \
        ...
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                              \
                           ...
                           static_cast<bool>(!!(__VA_ARGS__))))  // the expression here is never evaluated at runtime
...
#define INTERNAL_CATCH_NO_THROW(macroName, resultDisposition, ...)                                               \
    do {                                                                                                         \
        ...
        } catch (...) { catchAssertionHandler.useActiveException(); }                                            \
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                              \
...
#define INTERNAL_CATCH_THROWS(macroName, resultDisposition, ...)                                                 \
    do {                                                                                                         \
        ...
        if (catchAssertionHandler.allowThrows())                                                                 \
            ...
            } catch (...) { catchAssertionHandler.handle(Catch::ResultWas::Ok); }                                \
        else                                                                                                     \
            catchAssertionHandler.handle(Catch::ResultWas::Ok);                                                  \
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                              \
    } while (Catch::alwaysFalse())
    ...
    do {                                                                                                         \
        ...
        if (catchAssertionHandler.allowThrows())                                                                 \
            try {                                                                                                \
                ...
                catchAssertionHandler.handle(Catch::ResultWas::DidntThrowException);                             \
            } catch (exceptionType const&) { catchAssertionHandler.handle(Catch::ResultWas::Ok); } catch (...) { \
   ...
   public:
    ...
    Approx& margin(T const& newMargin) {
        ...
        CATCH_ENFORCE(marginAsDouble >= 0,
                      "Invalid Approx::margin: " << marginAsDouble << ", Approx::Margin has to be non-negative.");
...
TestCase makeTestCase(ITestInvoker* testCase, std::string const& className, std::string const& name,
            ...
            for (u_int m = 0; m < count; m++) {
                ...
                if (startsWith(methodName, "Catch_TestCase_")) {
                    ...
                    getMutableRegistryHub().registerTest(makeTestCase(
                        new OcMethod(cls, selector), className, name.c_str(), desc.c_str(), SourceLineInfo("", 0)));
                    noTestMethods++;
                }
            }
            free(methods);
...
struct StreamingReporterBase : IStreamingReporter {
    StreamingReporterBase(ReporterConfig const& _config) : m_config(_config.fullConfig()), stream(_config.stream()) {
        ...
        CATCH_ENFORCE(DerivedT::getSupportedVerbosities().count(m_config->verbosity()),
...
void AssertionHandler::handle(AssertionResultData const& resultData, ITransientExpression const* expr) {
    ...
    if (!assertionResult.isOk()) {
        ...
        m_shouldThrow      = getCurrentContext().getRunner()->aborting() ||
...
void RunContext::runCurrentTest(std::string& redirectedCout, std::string& redirectedCerr) {
    ...
    try {
        ...
        if (m_shouldReportUnexpected) {
            AssertionHandler(m_lastAssertionInfo.macroName, m_lastAssertionInfo.lineInfo,
...
TestCase makeTestCase(ITestInvoker* _testCase, std::string const& _className, std::string const& _name,
...
AutoReg::AutoReg(ITestInvoker* invoker, SourceLineInfo const& lineInfo, StringRef classOrMethod,
                 NameAndTags const& nameAndTags) noexcept {
    try {
...
struct ConsoleReporter : StreamingReporterBase<ConsoleReporter> {
    ...
    void benchmarkEnded(BenchmarkStats const& stats) override {
        ...
        m_tablePrinter << stats.iterations << ColumnBreak() << stats.elapsedTimeInNanoseconds << ColumnBreak()
                       << average << ColumnBreak();
    }
