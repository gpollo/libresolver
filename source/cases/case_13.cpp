...
pattern::tree::pattern case_13::get_pattern() const {
    /*
     * cmpl   $k6,k7(%r1)
     * ja     k5
     * mov    k4(%r1),%r1d
    ...
    return {
        /* clang-format off */
        {
            ...
            instruction::lea(
                ...
                .track_insns_ = {X86_INS_JA, X86_INS_JB}, /* used by other pattern */
            }
        }, {
            instruction::mov(
                operand::make_mem2(value::VALUE_4, reg::REG_1, size::QWORD),
            ...
            instruction::cmp(
                ...
                operand::make_mem2(value::VALUE_7, reg::REG_1, size::QWORD)
            ), {
...
std::string case_13::get_name() const {
    return "case-13";
}

std::unordered_set<uint64_t> case_13::evaluate(const context& context, const memory& memory) const {
    ...
    if (!utils::optional::all_value(rip0_opt, k1_opt, k2_opt, k3_opt, k4_opt, k6_opt, k7_opt)) {
        std::cerr << "[cases::case_13::evaluate] missing matched values" << std::endl;
        return {};
    }
    ...
    if (k4 != k7) {
        std::cerr << "[cases::case_13::evaluate] k4 is not equal to k7" << std::endl;
        return {};
    }
    ...
    for (unsigned int i = 0; i <= k6; i++) {
        ...
        if (!mem_opt.has_value()) {
            std::cerr << "[cases::case_13::evaluate] failed to read memory" << std::endl;
            return {};
        }
