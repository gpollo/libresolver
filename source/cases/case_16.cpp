...
pattern::tree::pattern case_16::get_pattern() const {
    /*
     * cmp    $k5,%r4d
     * ja     k4
     * mov    %r4d,%r3d
    ...
    return {
        /* clang-format off */
        {
            ...
            instruction::mov(
                operand::make_reg(reg::REG_4, size::DWORD),
...
std::string case_16::get_name() const {
    return "case-16";
}

std::unordered_set<uint64_t> case_16::evaluate(const context& context, const memory& memory) const {
    ...
    if (!utils::optional::all_value(rip0_opt, k1_opt, k2_opt, k3_opt, k5_opt)) {
        std::cerr << "[cases::case_16::evaluate] missing matched values" << std::endl;
        return {};
    }
    ...
    for (unsigned int i = 0; i <= k5; i++) {
        ...
        if (!mem_opt.has_value()) {
            std::cerr << "[cases::case_16::evaluate] failed to read memory" << std::endl;
            return {};
        }
