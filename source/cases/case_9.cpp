...
pattern::tree::pattern case_9::get_pattern() const {
    ...
    return {
        /* clang-format off */
        {
            ...
            instruction::mov(
                operand::make_mem2(value::VALUE_3, reg::REG_3, size::QWORD),
            ...
            instruction::cmp(
                ...
                operand::make_mem2(value::VALUE_6, reg::REG_3, size::QWORD)
            ), {
...
std::string case_9::get_name() const {
    return "case-9";
}
...
std::unordered_set<uint64_t> case_9::evaluate(const context& context, const memory& memory) const {
    ...
    if (!utils::optional::all_value(rip0_opt, k1_opt, k2_opt, k3_opt, k5_opt, k6_opt, k7_opt)) {
        std::cerr << "[cases::case_9::evaluate] missing matched values" << std::endl;
        return {};
    }
    ...
    if (k3 != k6) {
        std::cerr << "[cases::case_9::evaluate] k3 is not equal to k6" << std::endl;
        return {};
    }
    ...
    for (unsigned int i = 0; i <= k5; i++) {
        ...
        if (!mem_opt.has_value()) {
            std::cerr << "[cases::case_9::evaluate] failed to read memory" << std::endl;
            return {};
        }
