...
pattern::tree::pattern case_10::get_pattern() const {
    /*
     * cmp    $k5,%r1d
     * ja     k4
     * lea    k3(%rip),%r2
...
std::string case_10::get_name() const {
    return "case-10";
}

std::unordered_set<uint64_t> case_10::evaluate(const context& context, const memory& memory) const {
    ...
    if (!utils::optional::all_value(rip0_opt, k1_opt, k2_opt, k3_opt, k5_opt)) {
        std::cerr << "[cases::case_10::evaluate] missing matched values" << std::endl;
        return {};
    }
    ...
    for (unsigned int i = 0; i <= k5; i++) {
        ...
        if (!mem_opt.has_value()) {
            std::cerr << "[cases::case_10::evaluate] failed to read memory" << std::endl;
            return {};
        }
