...
case_12::case_12(arch arch) : case_base("case-12", arch) {}
...
pattern::tree::pattern case_12::get_pattern() const {
    ...
    return {
        jmp(reg(R1), {TRACK_R1}),
        add(reg(R2), reg(R1), {TRACK_R1}),
        movslq(mem4(K1, R2, R1, K2), reg(R1), {TRACK_INS_JA}),
        ja(imm(K3), {TRACK_INS_CMP, IGNORE_INS_JA}),
        cmp(imm(K4), reg(R1D), {IGNORE_R1, IGNORE_INS_CMP}),
